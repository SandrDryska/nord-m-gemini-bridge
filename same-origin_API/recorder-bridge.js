...
(function(){
  ...
  function readVar(name){
    try { return player ? player.GetVar(name) : undefined; } catch(_) { return undefined; }
  }
  ...
  function syncFromPlayer(){
    ...
    if (typeof vAutosend !== 'undefined') newState.autosend = toBool(vAutosend);
  ...
  async function send(){
    ...
    if (mode === 'voice') return sendAudio();
    // mixed: if audio exists -> audio, else text
    if (recordedAudioBlob) return sendAudio();
    return sendText();
  }
